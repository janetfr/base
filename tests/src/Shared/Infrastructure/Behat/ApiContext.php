...
namespace MyCoon\Tests\Shared\Infrastructure\Behat;

use Behat\Gherkin\Node\PyStringNode;
use Behat\Mink\Session;
use Behat\MinkExtension\Context\RawMinkContext;
use MyCoon\Tests\Shared\Infrastructure\Mink\MinkHelper;
use MyCoon\Tests\Shared\Infrastructure\Mink\MinkSessionRequestHelper;
...
final class ApiContext extends RawMinkContext
{
    ...
    /**
     * @Given I send a :method request to :url with :token
     */
    public function iSendARequestWithHeader($method, $url, $token): void
    {
        $this->request->sendRequestWithPyStringNodeAndHeader($method, $this->locatePath($url), $token, new PyStringNode([], 1));
    }
    ...
    /**
     * @Given I send a :method request to :url with :token and body:
     */
    public function iSendARequestToWithBodyAndHeader($method, $url, $token, PyStringNode $body): void
    {
        $this->request->sendRequestWithPyStringNodeAndHeader($method, $this->locatePath($url), $token,$body);
    }
